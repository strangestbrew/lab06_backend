...
const lookupInDB = function (handler, tableName) {
  const SQL = `SELECT * FROM ${tableName} WHERE location_id=$1;`;
...
function Handler(queryData, tableName, fetchFromAPI) {
...
//   let fetch = (request) => Location.fetchLocation(request.query.data);
//   let lookup = Location.lookupLocation(locationHandler);
//   let handler = new Handler(request.query.data);
